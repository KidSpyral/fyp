...
public class Sawm extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
...
// Calculate prayer times for a whole month (you can implement this part)
    ...
    private void updateCurrentTime() {
        ...
        Date maghribTime = prayerTimes.maghrib;

        // Find the time difference between the current time and Maghrib
        long currentTimeMillis = System.currentTimeMillis();
        long maghribTimeMillis = maghribTime.getTime();
        long timeDifferenceMillis = maghribTimeMillis - currentTimeMillis;

        // Check if Maghrib has already occurred for the day
        if (timeDifferenceMillis > 0) {
            // Maghrib is yet to occur, convert time difference to hours, minutes, and seconds
            ...
            timeTextView.setText(countdownText);

            // Apply the animation to the countdown text
            Animation blinkAnimation = AnimationUtils.loadAnimation(this, R.anim.blink_anim);
            timeTextView.startAnimation(blinkAnimation);
        } else {
            // Maghrib has already occurred for the current day, reset the text
            timeTextView.setText("Iftar Time");
    ...
    private void updateNextPrayerTime() {
        ...
        // Find the current time and determine if Maghrib has already occurred
        Date currentTime = new Date();
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(currentTime);
        int currentMinutesSinceMidnight = (calendar.get(Calendar.HOUR_OF_DAY) * 60) + calendar.get(Calendar.MINUTE);

        calendar.setTime(maghribTime);
        int maghribMinutesSinceMidnight = (calendar.get(Calendar.HOUR_OF_DAY) * 60) + calendar.get(Calendar.MINUTE);

        if (maghribMinutesSinceMidnight > currentMinutesSinceMidnight) {
            // Maghrib is yet to occur
            Date nextMaghribTime = maghribTime;
            String nextMaghribName = "Maghrib";



            nextPrayerNameTextView.setText(nextMaghribName); // Update prayer name
            nextPrayerTimeTextView.setText(formatter.format(nextMaghribTime)); // Update prayer time
            ...
            notificationIntent.putExtra("prayer_name", nextMaghribName);
            sendBroadcast(notificationIntent);
        } else {
            // Maghrib has already occurred for the current day,
            // calculate the timings for the next day's Maghrib
            Calendar nextDay = Calendar.getInstance();
            nextDay.add(Calendar.DAY_OF_MONTH, 1);
            dateComponents = DateComponents.from(nextDay.getTime());
            prayerTimes = new PrayerTimes(coordinates, dateComponents, calculationMethod.getParameters());
            Date nextMaghribTime = prayerTimes.maghrib;
            String nextMaghribName = "Maghrib";

            nextPrayerNameTextView.setText(nextMaghribName); // Update prayer name
            nextPrayerTimeTextView.setText(formatter.format(nextMaghribTime)); // Update prayer time

            calendar.setTime(nextMaghribTime);
